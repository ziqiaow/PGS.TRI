...
## Example Analysis
We provide a simple example for running our proposed method using simulated data. The R function of the proposed method is in [PGS-TRI](https://github.com/ziqiaow/PGS.TRI/blob/main/R/PGS-TRI.R). The R function to simulate data is available here [simulation](https://github.com/ziqiaow/PGS.TRI/blob/main/R/simulation.R). We also provide the R function to run the polygenic TDT (pTDT) test [pTDT](https://github.com/ziqiaow/PGS.TRI/blob/main/R/pTDT.R) (originally proposed by Weiner et al, Nat Genet. 2017). For PGSxE analysis, the case-only method is also implemented here [case-only](https://github.com/ziqiaow/PGS.TRI/blob/main/R/case-only.R) (Allison et al, AJE 2019; Wang et al, AJE 2024).
